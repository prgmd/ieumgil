...
public class KakaoLocalClient {
    ...
    public List<KakaoPlaceResponse.Document> searchByKeyword(String query, Double lat, Double lng) {
        return callKakao(() -> {
            ...
            return URI.create(properties.baseUrl() + "/v2/local/search/keyword.json?" + qs);
        }, KakaoPlaceResponse.class, "카카오 로컬 검색 실패", response ->
                response == null || response.documents() == null ? List.of() : response.documents());
    }
    ...
    public List<KakaoPlaceResponse.Document> searchByKeywordInRect(
        ...
        return callKakao(() -> {
            ...
            return URI.create(properties.baseUrl() + "/v2/local/search/keyword.json?" + qs);
        }, KakaoPlaceResponse.class, "카카오 로컬 범위 검색 실패", response ->
                response == null || response.documents() == null ? List.of() : response.documents());
    }

    public Optional<KakaoAddressResponse.Document> coord2Address(double lat, double lng) {
        return callKakao(
                () -> URI.create(properties.baseUrl() + "/v2/local/geo/coord2address.json?x=" + lng + "&y=" + lat),
                KakaoAddressResponse.class, "카카오 좌표→주소 변환 실패", response ->
                        response == null || response.documents() == null || response.documents().isEmpty()
                                ? Optional.empty()
                                : Optional.of(response.documents().get(0)));
    }

    /** 주소 → 좌표(정지오코딩). {@code coord2Address}의 반대 방향이다 */
    public Optional<KakaoGeocodeResponse.Document> addressSearch(String address) {
        return callKakao(
                () -> URI.create(properties.baseUrl() + "/v2/local/search/address.json"
                        + "?query=" + URLEncoder.encode(address, StandardCharsets.UTF_8)),
                KakaoGeocodeResponse.class, "카카오 주소→좌표 변환 실패", response ->
                        response == null || response.documents() == null || response.documents().isEmpty()
                                ? Optional.empty()
                                : Optional.of(response.documents().get(0)));
    }
    ...
    public Optional<KakaoWalkingRouteResponse.Properties> getWalkingRoute(
        ...
        return callKakao(
                () -> URI.create(properties.baseUrl() + "/v2/routing/walk"
                        + "?start_x=" + startLng + "&start_y=" + startLat
                        + "&end_x=" + endLng + "&end_y=" + endLat),
                KakaoWalkingRouteResponse.class, "카카오 도보 길찾기 실패", response ->
                        response == null || response.route() == null || !"OK".equals(response.status())
                                ? Optional.empty()
                                : Optional.of(response.route().properties()));
    }

    public Optional<KakaoDirectionsResponse.Summary> getDrivingRoute(
            double startLat, double startLng, double endLat, double endLng) {
        return callKakao(
                () -> URI.create(DIRECTIONS_BASE_URL + "/v1/directions"
                        + "?origin=" + startLng + "," + startLat
                        + "&destination=" + endLng + "," + endLat
                        + "&priority=TIME"),
                KakaoDirectionsResponse.class, "카카오모빌리티 자동차 길찾기 실패", response -> {
    ...
    /**
     * 카카오 로컬/모빌리티 호출의 공통 try/catch + 인증 헤더 부착을 한곳에 모은다.
     * URI 생성({@link URLEncoder} 인코딩·{@link URI#create})을 try 안에서 실행해
     * 기존과 동일하게 {@link IllegalArgumentException}까지 잡는다.
     */
    private <T, R> R callKakao(
            java.util.function.Supplier<URI> uriSupplier, Class<T> type, String failMessage,
            java.util.function.Function<T, R> unwrap) {
        try {
