...
public class FestivalRecommendationTool {
    ...
    public FestivalRecommendationTool(LocalDate tripStartDate, LocalDate tripEndDate,
    ...
    @Tool(description = """
            ...
            Determine which province (시/도) the current trip destination belongs to and pass its short name as `region`. Examples: 전주→전북, 서귀포/성산일출봉→제주, 강릉→강원, 여수→전남, 경주→경북, 수원→경기.
            Valid values are the 17 provinces: 서울 부산 대구 인천 광주 대전 울산 세종 경기 강원 충북 충남 전북 전남 경북 경남 제주. Pass one of these exactly — never a city, district, or landmark name.
            The travel dates are handled automatically from the current project, so call it directly and immediately — never ask the user for the travel dates first.
            Do NOT call it for requests about restaurants, cafes, lodging, or transportation — those are not festivals/events.
            Returns the festivals/events for the given region and the current project's travel period, along with the region name used; if the user asked about a different region, note the result may differ.
            ...
            """)
    public FestivalRecommendationResult findFestivalsForCurrentTrip(String region) {
        RegionCode regionCode = RegionCode.findByName(region).orElse(null);
        if (regionCode == null) {
            return FestivalRecommendationResult.unavailable(region);
        }
        try {
            ...
            log.warn("festival tool call failed for region={}", region, e);
