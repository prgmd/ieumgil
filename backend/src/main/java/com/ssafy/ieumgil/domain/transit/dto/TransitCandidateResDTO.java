...
public class TransitCandidateResDTO {
    ...
    public record Candidate(
             ...
             *
             * <p>현재는 어떤 후보도 이 필드를 채우지 않는다 — 예전의 페리 경고 + ESTIMATE 강등을
             * 폐기했기 때문이다. 차로 갈 수 없는 목적지(ODsay 시외 경로가 전부 항공)에서는 경고를
             * 붙이는 대신 자차·택시 후보를 <b>아예 만들지 않는다</b>({@code LandReachability}) —
             * 527,600원짜리 제주행 택시 요금에 경고만 얹어 내보내는 것은 여전히 거짓을 말하는
             * 것이다. ODsay가 대중교통 경로 자체를 주지 않는 구간에서 {@code NO_ROUTE}가 되는 것은
             * 대중교통 후보뿐이다. 다른 신뢰도 문제가 생기면 이 필드를 재사용한다.
             */
            String caution,
