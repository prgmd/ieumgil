...
공통: `Authorization: Bearer {accessToken}` (재발급 제외). 응답 래퍼 `{ "data": ..., "error": null }`. 오류는 `{ "error": { "code": "GROUP_FULL", "message": "..." } }` 형태로 코드 구분(MY-05의 실패 사유 구분 등에 사용).

### 2-1. 인증 `AUTH`

| 메서드 | 경로 | 설명 | 요청 | 응답 |
|---|---|---|---|---|
| GET | `/oauth2/authorization/kakao` | 카카오 인가 요청 리다이렉트 | - | 302 |
| GET | `/oauth2/authorization/naver` | 네이버 인가 요청 리다이렉트 | - | 302 |
| GET | `/login/oauth2/code/{provider}` | 콜백(Spring Security 처리) → 프론트로 리다이렉트 + Refresh 쿠키 세팅 | code | 302 → `/oauth/redirect` |
| POST | `/api/auth/token` | 콜백 직후 Access 토큰 교환 | Cookie: refresh | `{accessToken, member}` |
| POST | `/api/auth/reissue` | Access 재발급 (30분 만료 시) | Cookie: refresh | `{accessToken}` |
| POST | `/api/auth/logout` | Refresh 무효화(Redis 블랙리스트) | - | 204 |
...
토큰 정책(AUTH-04): Access 30분 — 응답 바디로만 전달, 프론트는 메모리 보관. Refresh 14일 — `HttpOnly; Secure; SameSite=Lax` 쿠키.
...
| 기능 | 기술 | 관련 ID |
|---|---|---|
| 소셜 로그인 | `spring-boot-starter-oauth2-client` (카카오·네이버 provider 수동 등록), `jjwt` 0.12.x | AUTH-02~04 |
