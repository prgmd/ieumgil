...
| 메서드 | 경로 | 설명 | 요청 | 응답 |
|---|---|---|---|---|
| POST | `/api/auth/login/kakao` | 카카오 인가 코드로 로그인/회원가입. BE가 카카오 토큰 발급 API·사용자정보 API 직접 호출 → `provider`+`provider_id`로 기존 회원 조회, 없으면 신규 생성 → JWT 발급 | `{code}` | `{accessToken, refreshToken, member}` |
| POST | `/api/auth/login/naver` | 네이버 버전, 동일 흐름 | `{code}` | `{accessToken, refreshToken, member}` |
| POST | `/api/auth/reissue` | Access 재발급 (30분 만료 시) | `{refreshToken}` | `{accessToken}` |
| POST | `/api/auth/logout` | Refresh 무효화(Redis 블랙리스트) | `{refreshToken}` | 204 |
...
토큰 정책(AUTH-04): Access 30분 — 응답 바디로만 전달, 프론트는 메모리 보관. Refresh 14일 — 응답 바디로 전달, 프론트는 localStorage에 저장.
