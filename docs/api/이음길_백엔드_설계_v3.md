...
| 메서드 | 경로 | 설명 | 요청 | 응답 |
|---|---|---|---|---|
| POST | `/api/auth/login/kakao` | 카카오 인가 코드로 로그인/회원가입. BE가 카카오 토큰 발급 API·사용자정보 API 직접 호출 → `provider`+`provider_id`로 기존 회원 조회, 없으면 신규 생성 → JWT 발급 | `{code}` | `result`: `{userId, nickname, profileImageUrl, accessToken, isNewUser}` + `Set-Cookie: refreshToken` |
| POST | `/api/auth/login/naver` | 네이버 버전, 동일 흐름 | `{code}` | 〃 |
| POST | `/api/auth/refresh` | Access 재발급 (30분 만료 시). RTR — refreshToken도 함께 교체 | Cookie: `refreshToken` | `result`: `{accessToken}` + `Set-Cookie: refreshToken` |
| POST | `/api/auth/logout` | Refresh 무효화(Redis 삭제) + refreshToken 쿠키 만료 | - | 200 + `Set-Cookie: refreshToken`(Max-Age=0) |
| POST | `/api/auth/test-token?memberId=` | **dev 전용(`@Profile("local")`)** — 시드 멤버용 진짜 JWT 발급. Swagger에서 카카오 콜백 없이 "멤버 n으로 로그인" 상태를 즉시 구성, 동시 편집 테스트 시 멤버 1·2 토큰을 각각 받아 두 탭으로 검증 | - | `{accessToken}` |
| GET | `/api/members/me` | 내 정보 | - | `{id, nickname, profileImg}` |
| DELETE | `/api/members/me` | 회원 탈퇴 — §1-2 member 탈퇴 정책 수행 | - | 204 |

토큰 정책(AUTH-04): Access 30분 — 응답 바디로만 전달, 프론트는 메모리 보관. Refresh 14일 — `HttpOnly; Secure; SameSite=Lax; Path=/api/auth` 쿠키. 프론트는 쿠키를 JS로 다루지 않으며, 인증 요청 시 `withCredentials: true`로 브라우저가 자동 전송한다. 재발급 시 저장된 토큰과 불일치하면 탈취로 간주해 해당 사용자의 refresh를 삭제하고 재로그인을 유도한다.

### 2-2. 그룹 `MY` `GRP`

| 메서드 | 경로 | 설명 | 요청 바디 | 비고 |
|---|---|---|---|---|
| GET | `/api/groups` | 내 그룹 목록 (그룹명, 멤버 수, **전체** 프로젝트 수 — 완료 여부 무관, 멤버 아바타) | - | MY-01 |
...
| POST | `/api/groups/join` | 초대 코드 입장 | `{inviteCode}` | 오류 코드: `COMMON400_1`(형식) / `GROUP404_1`(없음) / `GROUP410`(만료) / `GROUP409`(이미 멤버) / `GROUP409_1`(정원) |
...
| 메서드 | 경로 | 설명 | 비고 |
|---|---|---|---|
| POST | `/api/projects/{projectId}/keywords` | 키워드 저장 + 후보 블록 자동 생성(BOT-02) | `{keywords[]}` 최대 5개. **재입력 가능(v3 확인)** — 재생성 시 기존 후보 블록은 그룹 자산이므로 유지하고 신규 생성분을 추가(중복 담기는 MAP-03이 어차피 허용). **Redis 분산 락은 동시 중복 실행 방지용**. 생성 결과는 BLOCK_CREATED op 다건 브로드캐스트 |
| POST | `/api/projects/{projectId}/bot/chat` | 대화형 추천(BOT-03) | `{message, mapContext?}` → 응답 `{reply, candidates[]}` — 후보는 서버가 곧바로 후보 블록으로 생성(BOT-04). 4초 초과 대비 프론트 로딩 표시 |

LLM 파이프라인: 사용자 질의 + 컨텍스트(키워드·기간·현재 체인 요약) → LLM이 장소 후보 명사 추출 → **카카오 로컬 검색으로 실좌표 검증** → 검증 통과 건만 블록 생성. LLM 환각 좌표를 그대로 쓰지 않는 것이 핵심.
